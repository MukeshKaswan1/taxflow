...
export default function LaunchJobForm() {
  ...
  return (
    <Card className="p-6 bg-white border border-slate-200 shadow-sm" glow>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 relative z-10">
        <Field label="Target PAN *">
          <Input
            ...
            maxLength={10}
            required
            className="font-mono tracking-[0.2em] text-slate-800"
          />
        </Field>
        ...
        <Field label="Registration Type">
          <div className="flex bg-slate-100 rounded-lg p-1 border border-slate-200">
            <button
              ...
              className={`flex-1 py-2 text-sm rounded-md transition-all duration-300 ${!isOthers ? 'bg-blue-600 text-white font-bold' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50/50'}`}
            >
              Taxpayer
            </button>
            <button
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          <button
            ...
            className="w-full flex justify-between items-center p-3 bg-slate-50 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 transition-colors"
          >
          ...
          {showAdvanced && (
            <div className="p-4 flex flex-col gap-3 bg-white border-t border-slate-150">
              ...
              <Field label="Gender *">
                <div className="flex flex-wrap gap-3">
                  {['Male', 'Female', 'Transgender'].map((g) => (
                    <label key={g} className="flex items-center gap-1.5 cursor-pointer text-sm text-slate-600 hover:text-slate-800">
                      <input type="radio" name="gender" value={g} checked={gender === g} onChange={() => setGender(g)} className="accent-blue-600" />
                      {g}
                    </label>
                  ))}
                </div>
              ...
              <Field label="Residential Status *">
                <div className="flex flex-wrap gap-4">
                  {['Resident', 'Non Resident'].map((s) => (
                    <label key={s} className="flex items-center gap-1.5 cursor-pointer text-sm text-slate-600 hover:text-slate-800">
                      <input type="radio" name="residentialStatus" value={s} checked={residentialStatus === s} onChange={() => setResidentialStatus(s)} className="accent-blue-600" />
                      {s}
                    </label>
                  ))}
                </div>
              ...
              <div className="border-t border-slate-200 pt-3 mt-1">
                <p className="text-xs text-slate-400 mb-3 uppercase tracking-wider font-semibold">Contact Details</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="border-t border-slate-200 pt-3 mt-1">
                <p className="text-xs text-slate-400 mb-3 uppercase tracking-wider font-semibold">Postal Address</p>
        ...
        <Button type="submit" loading={loading} className="mt-2 w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all shadow-md hover:shadow-lg">
